...
export function CloudClubs() {
  ...
  async function loadClubs() {
    ...
    const [clubResult, occurrenceResult] = await Promise.all([
      ...
      supabase.rpc('get_my_club_occurrences', { input_from: today, input_to: addDays(today, 28) }),
    ])
    if (clubResult.error || occurrenceResult.error) {
    ...
    setOccurrences((occurrenceResult.data ?? []) as ClubOccurrence[])
    setState('ready')
  }
  ...
  return <section className="screen"><div className="screen-heading"><div><p className="eyebrow">После уроков</p><h1>Кружки</h1></div></div>
    ...
    {state === 'error' && <div className="auth-message error" role="alert"><p>Не получилось загрузить кружки. Проверь интернет и попробуй ещё раз.</p><button type="button" className="secondary-button" onClick={() => void loadClubs()}>Повторить</button></div>}
