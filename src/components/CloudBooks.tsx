...
export function CloudBooks() {
  ...
  return <section className="screen"><div className="screen-heading"><div><p className="eyebrow">Читательский дневник</p><h1>Книги</h1></div></div>
    ...
    {editingBook && draft && <div className="sheet-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) void closeDiary() }}><section className="sheet-panel reading-diary-sheet" role="dialog" aria-modal="true" aria-labelledby="diary-title"><div className="sheet-heading"><div><p className="eyebrow">Читательский дневник</p><h2 id="diary-title">{editingBook.title}</h2></div><button ref={closeButtonRef} type="button" className="sheet-close" aria-label="Закрыть дневник" onClick={() => void closeDiary()}>×</button></div><div className="reading-diary-scroll"><p>{editingBook.author}</p><form className="auth-form" onSubmit={saveDiary}>
      ...
      <div className="parent-form-grid"><div><label htmlFor="book-started">Начал читать</label><span className="sheet-native-field"><input id="book-started" type="date" value={draft.startedOn} onChange={(event) => updateDraft({ startedOn: event.target.value })} /></span></div><div><label htmlFor="book-finished">Закончил читать</label><span className="sheet-native-field"><input id="book-finished" type="date" value={draft.finishedOn} onChange={(event) => updateDraft({ finishedOn: event.target.value })} /></span></div></div>
